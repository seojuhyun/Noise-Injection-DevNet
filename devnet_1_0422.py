...
def deviation_loss(y_true, y_pred):
    ...
    ref = K.constant(np.random.normal(loc = 0., scale= 1.0, size = 5000) , dtype='float32')
    dev = (y_pred - K.mean(ref)) / K.std(ref)
    inlier_loss = K.abs(dev) 
    outlier_loss = K.abs(K.maximum(confidence_margin - dev, 0.))
    #return K.mean((1 - y_true) * inlier_loss + y_true * outlier_loss)inject_noise
    return K.mean((1 - K.cast(y_true, dtype='float32')) * inlier_loss + K.cast(y_true, dtype='float32') * outlier_loss)
...
def plot_mean_roc_pr(y_tests, y_scores, output_dir="./"):
    import matplotlib.pyplot as plt
    ...
    for y_test, y_score in zip(y_tests, y_scores):
        ...
        rec, prec, _ = precision_recall_curve(y_test, y_score)
        tpr_list.append(np.interp(np.linspace(0, 1, 100), fpr, tpr))
        fpr_list.append(np.linspace(0, 1, 100))
        prec_list.append(np.interp(np.linspace(0, 1, 100), rec[::-1], prec[::-1]))
        rec_list.append(np.linspace(0, 1, 100))
    ...
    mean_auc_pr = auc(mean_rec, mean_prec)

    plt.figure()
    plt.plot(mean_fpr, mean_tpr, label=f'Mean ROC (AUC = {mean_auc_roc:.2f})')
    plt.plot([0, 1], [0, 1], linestyle='--', color='gray')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('Proposed Average ROC Curve')
    plt.legend()
    plt.grid(True)
    roc_path = os.path.join(output_dir, "mean_roc_curve.png")
    plt.savefig(roc_path)
    plt.close()
    print(f"✅ ROC curve saved to {roc_path}")

    plt.figure()
    plt.plot(mean_rec, mean_prec, label=f'Mean PR (AUC = {mean_auc_pr:.2f})')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Proposed Average Precision-Recall Curve')
    plt.legend()
    plt.grid(True)
    pr_path = os.path.join(output_dir, "mean_pr_curve.png")
    plt.savefig(pr_path)
    plt.close()
...
def save_results(dataset_name, script_name, auc_roc, auc_pr, f1, output_file="results_10R_11.csv"):
    """
    모델 평가 결과(AUC-ROC, AUC-PR)를 CSV 파일에 저장 (파일이 있으면 append)
    
    Parameters:
        dataset_name (str): 데이터셋 이름
        script_name (str): 실행된 파일 이름 (예: devnet_0.py)
        auc_roc (float): 평균 AUC-ROC
        auc_pr (float): 평균 AUC-PR
        output_file (str): 결과 저장 파일 (기본값: results.csv)
    """
    ...
    with open(output_file, mode='a', newline='') as f:
        ...
        # 파일이 없으면 헤더 추가
        if not file_exists:
            writer.writerow(["Dataset", "Script", "AUC-ROC", "AUC-PR", "F1-score"])

        # 결과 추가
        writer.writerow([dataset_name, script_name, auc_roc, auc_pr, f1])

    print(f"✅ Results saved to {output_file}")

# 예제: 실행된 파일 이름을 자동으로 가져오기
script_name = os.path.basename(__file__)  # 실행된 파일명 (예: devnet_0.py)
...
def run_devnet(args):
    ...
    # ✅ 고정된 10개의 시드 리스트
    seed_list = [971, 879, 796, 623, 18, 135, 66, 933, 200, 805]
    runs = len(seed_list)
    #random_seed = args.ramdn_seed
    
    for nm in names:
        filename = nm.strip()  # 데이터셋 이름 가져오기
        
        #runs = args.runs
        rauc = np.zeros(runs)
        ap = np.zeros(runs)  
        f1s = np.zeros(runs)  # ✅ F1-score 추가
        ...
        for i in np.arange(runs):  
            current_seed = seed_list[i]  # seed는 여기서 설정
            print(f"\n🔁 Run {i + 1} with seed {current_seed}")
            ...
            if data_format == 0:                
                ...
                noises = inject_noise(outliers, n_noise, current_seed)
                x_train = np.append(x_train, noises, axis = 0)
                y_train = np.append(y_train, np.ones((noises.shape[0], 1)))   #-----여기--------
                ...
                noises = inject_noise_sparse(outliers, n_noise, current_seed)
                x_train = vstack([x_train, noises])
                y_train = np.append(y_train, np.zeros((noises.shape[0], 1)))   #-----여기--------
        ...
        print("average AUC-ROC: %.3f, average AUC-PR: %.3f, average F1: %.3f" % (mean_auc, mean_aucpr, mean_f1))
        ...
        writeResults(filename+'_'+str(network_depth), x.shape[0], x.shape[1], n_samples_trn, n_outliers_org, n_outliers,
         ...
         # ✅ 결과 저장
        save_results(filename, script_name, mean_auc, mean_aucpr, mean_f1)
        
        # ✅ 평균 ROC/PR 그래프 그리기
        plot_mean_roc_pr(y_tests_all, y_scores_all)
