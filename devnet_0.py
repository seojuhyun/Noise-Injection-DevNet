...
def plot_mean_roc_pr(y_tests, y_scores, output_dir="./"):
    ...
    plt.legend()
    plt.grid(True)
    roc_path = os.path.join(output_dir, "mean_roc_curve0Cre.png")
    ...
    plt.legend()
    plt.grid(True)
    pr_path = os.path.join(output_dir, "mean_pr_curve0Cre.png")
    plt.savefig(pr_path)
    plt.close()
    print(f"✅ PR curve saved to {pr_path}")
    
def save_results(dataset_name, script_name, auc_roc, auc_pr, output_file="results_10R_00.csv"):
    """
    ...
    with open(output_file, mode='a', newline='') as f:
        ...
        # 파일이 없으면 헤더 추가
        if not file_exists:
            writer.writerow(["Dataset", "Script", "AUC-ROC", "AUC-PR"])

        # 결과 추가
        writer.writerow([dataset_name, script_name, auc_roc, auc_pr])
...
def run_devnet(args):
    ...
    for nm in names:
        ...
        print("average AUC-ROC: %.3f, average AUC-PR: %.3f" % (mean_auc, mean_aucpr))    
        ...
        writeResults(filename+'_'+str(network_depth), x.shape[0], x.shape[1], n_samples_trn, n_outliers_org, n_outliers,
         ...
         # ✅ 결과 저장
        save_results(filename, script_name, mean_auc, mean_aucpr)
        # ✅ 평균 ROC/PR 그래프 그리기
